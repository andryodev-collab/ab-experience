export function initMotion({ bhState, reduced, getIsMobile, onWordSignal }) {
  ...
  // Header is a navigation layer, never a scroll gate. It also survives a direct scrollbar jump.
  ...
  // Hero physically leaves the viewport while the universe keeps travelling behind it.
  ...
  // Holograms are real document content. On coarse/mobile screens they stay centered
  // and use frontal depth only, so no animation can push an edge outside the viewport.
  huds.forEach((card,i)=>{
  ...
  // Concept words are written letter by letter by text-reveal.js.
  // This layer only lets the completed word recede once it has been read.
  ...
  // Timeline: animation follows the content instead of holding it.
